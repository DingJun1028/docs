...
- Fixed query generation for certain usages of the `$or` filter, impacting only the beta `core-api` server.
</Update>
...
- Removed references to the obsolete 'thread context' column from documentation.
- Restored A2A support that was previously disabled.
</Update>
