...
- Introduced usage reporting support for the self-hosted Agent Builder.
- Standardized error status codes for assistants CRUD operations to return 404 for nonexistent graph IDs.
</Update>

<Update label="2026-01-12" tags={["agent-server"]}>
## v0.6.33

- Introduced a unique `graph_id` for graphs to prevent name collisions and enhance graph lookup reliability.
</Update>

<Update label="2026-01-11" tags={["agent-server"]}>
## v0.6.32

- Fixed query generation for certain usages of the `$or` filter; only impacts FF_USE_CORE_API users of the beta `core-api` server (unreleased).
</Update>
