...
With KEDA enabled, the queue services will automatically scale up when the Redis backlog grows and scale down when the backlog is processed. This is especially useful for handling variable trace ingestion loads without over-provisioning resources.

<Warning>
If you are upgrading from HPA (Horizontal Pod Autoscaler) to KEDA, you must manually delete the existing HPAs before enabling KEDA autoscaling:

```bash
kubectl delete hpa langsmith-queue -n <namespace>
kubectl delete hpa langsmith-ingest-queue -n <namespace>
```
</Warning>
...
## KEDA autoscaling recommended for queue services
queue:
  autoscaling:
    keda:
      enabled: true
